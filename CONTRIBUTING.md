...
This repo packages [Bisq](https://github.com/bisq-network/bisq) for StartOS.
...
Bisq is built into the image from the upstream `.deb` published at `bisq.network/downloads/v<version>/`, verified against the pinned PGP key. There is no `dockerTag` in the manifest.

1. Bump `BISQ_VERSION` (and `BISQ_PGP_KEY` if upstream rotated signers) in `Dockerfile`.
